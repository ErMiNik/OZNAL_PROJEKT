...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      # Run
      hr(),
      actionButton("btn_run", "Run Model", width = "100%",
                    style = "background-color: #337ab7; color: white; font-weight: bold;")
    ),
...
server <- function(input, output, session) {
  ...
  output$feature_checkboxes <- renderUI({
    ...
    checkboxGroupInput("selected_features", NULL, choices = feats, selected = NULL)
  })
  ...
  observeEvent(input$preset_manual, {
    updateCheckboxGroupInput(session, "selected_features", selected = MANUAL_PRESET)
  })
  ...
  observeEvent(input$btn_run, {
    ...
    if (length(feats) == 0) {
      showNotification("Select at least one feature", type = "error")
      return()
    }
    ...
    if (input$model_type == "logistic") {
      # Logistic Regression
      withProgress(message = "Fitting Logistic Regression...", {
        model <- tryCatch(
      ...
      if (is.null(model)) {
        showNotification("Model failed to fit", type = "error")
        return()
      }
      ...
      # Decision Tree   
      withProgress(message = "Fitting Decision Tree...", {
        model <- tryCatch(
          rpart(fml, data = d$train, method = "class",
    ...
    model_results(list(
      model = model, model_type = input$model_type,
  ...
  observeEvent(input$btn_youden, {
    res <- model_results()
    if (is.null(res)) {
      showNotification("Run a model first", type = "error")
      return()
    }
  ...
  compute_metrics <- function(prob, actual, threshold) {
    ...
    list(tab = tab, accuracy = accuracy, precision = precision,
         recall = recall, specificity = specificity, f1 = f1)
  }
  

  # SHARED TABS (both models)
  
  
  # Tab: Metrics   
  output$metrics_display <- renderUI({
    res <- model_results()
    if (is.null(res)) return(h4("Select features and click 'Run Model' to see results."))
  ...
  output$confusion_plot <- renderPlot({
    ...
    tab_df$Label <- paste0(tab_df$Freq, "\n",
      ifelse(tab_df$Predicted == "1" & tab_df$Actual == "1", "(TP)",
      ifelse(tab_df$Predicted == "0" & tab_df$Actual == "0", "(TN)",
  ...
  output$roc_plot <- renderPlot({
    ...
    ggplot() +
      ...
      geom_line(data = train_df, aes(x = 1 - specificity, y = sensitivity, color = "Train"),
                linewidth = 1, alpha = 0.5) +
      geom_line(data = test_df, aes(x = 1 - specificity, y = sensitivity, color = "Test"),
                linewidth = 1.2) +
      scale_color_manual(
  ...
  output$coef_plot <- renderPlot({
    res <- model_results()
    if (is.null(res) || res$model_type != "logistic") return(NULL)
  ...
  output$tree_plot <- renderPlot({
    res <- model_results()
    if (is.null(res) || res$model_type != "tree") return(NULL)
    
    rpart.plot(res$model,
               type = 4,
               extra = 104,
               under = TRUE,
               roundint = FALSE,
